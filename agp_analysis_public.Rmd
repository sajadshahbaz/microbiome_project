...
tdata.ps<-tdata
pscnt<-min(tdata[tdata>0])/10
tdata.ps[,]<-pmax(unlist(tdata.ps),pscnt)
...
samples_chosen_names<- rep(NA, length(donors))
...
for (i in 1:length(donors)) {
 ...
 #taxa
 taxa<-as.matrix(tdata[samples,,drop=F])
 taxa.ps<-as.matrix(tdata.ps[samples,,drop=F]) 
 taxonomy.mean[i,]<-colMedians(taxa) # median vector x_m of x_1, x_2 ... x_n, x_i<- abundance vector of i-th sample
 taxa.ps.mean<- colMedians(taxa.ps)
 which.sample<-which.min(colSums(log(t(taxa.ps)/taxa.ps.mean))) # arg min_i : || log(x_i) - log(x_m) ||_2
 taxonomy.mean[i,]<- taxa[which.sample,]
 samples_chosen_names[i]<- rownames(taxa)[which.sample]
...
samples_chosen_names<- samples_chosen_names[mask.meta]
...
save(meta.mean.final,decision.mean.final,taxonomy.mean.final,binary.mean.final, samples_chosen_names, file='mean_data.RData')
} else {load("old_mean_data.RData")}
#sanity correctness check:
#tsub<-tdata[samples_chosen_names, colnames(taxonomy.mean.final)]
#stopifnot(all((tsub/rowSums(tsub)*10000)==taxonomy.mean.final))
...
P<-ggplot(long_perf[long_perf$Method!="md12DpInt",], aes(x = Method, y = Value, fill = Method)) +
...
ggsave("figures/agp_boxplots.pdf", P)
print(P)
```
...
global_basis<- Reduce(union, lapply(global_MDFS, function(x) x$rel_set)) 
global_basis<- union(global_basis, global_U$rel_set)


frequent_<- list(over98=.98*n_repeats, over95= .95*n_repeats, over90=.9*n_repeats)
...
print("over 90 vs global_basis: ")
print("union: ")
length(union(colnames(features_AP_filtered90), global_basis))
print("intersection: ")
length(intersect(colnames(features_AP_filtered90), global_basis))
print("o90 - global_basis: ")
length(setdiff(colnames(features_AP_filtered90), global_basis))
print("global_basis - o90: ")
length( setdiff(global_basis, colnames(features_AP_filtered90)))

stopifnot( 
length(setdiff(colnames(features_AP_filtered90), global_basis))==0
)

feature_absence_presence[, global_basis] -> global_basis_freq
...
```{r}
#| label: viz_globalbasis
#| dependson: freq_filter
#| cache: true

global_basis_viz<- global_basis_freq
colnames(global_basis_viz)<- sapply(strsplit(colnames(global_basis_viz),split="\\."), function(x) x[[length(x)]])
global_basis_viz<-t(global_basis_viz) |> as.data.frame()
global_basis_viz[ do.call(order, global_basis_viz),]
```
...
for (j in c(1:n_repeats) ){
  ...
  abundance_per_rep[j,]= colSums(plausible_ab_at_j)/colSums(presence_0_at_j)
  presence_0[j,]=  colMeans(presence_0_at_j)
  presence_m[j,]=  colMeans(presence_m_at_j)
  abundance_per_repH[j,]=colSums(plausible_ab_at_j[y[subsample[[j]]$keep]==0, ])/colSums(presence_0_at_j[y[subsample[[j]]$keep]==0, ])
  abundance_per_repD[j,]=colSums(plausible_ab_at_j[y[subsample[[j]]$keep]==1, ])/colSums(presence_0_at_j[y[subsample[[j]]$keep]==1, ])
...
vpm0<- partners_m0
named_partners_m0<- partners_m0
rownames(named_partners_m0)<- plausible
colnames(named_partners_m0)<- colnames(tdata)
...
saveRDS(rownames(named_partners_m0[ rowMaxs(named_partners_m0)>=450, colMaxs(named_partners_m0)>= 450 ] ), "plausible_relevant2D_and_synergistic.rds")
```

```{r}
#frequency of being a partner:
freq_and_partners[, 3: ncol(freq_and_partners) ] |> colMaxs()
```
...
vpm0<- vpm0[,order(colSums(vpm0),decreasing=TRUE)]
vpm0<- vpm0[order(rowSums(vpm0), decreasing = TRUE),]
vpm0<- vpm0[, colMaxs(vpm0)>450]
vpm0<- vpm0[rowMaxs(vpm0)>450, ]
vpm0_flat<-melt(vpm0)
colnames(vpm0_flat)<-c("rel","partner","freq")
ggplot(vpm0_flat, aes(x=partner,y=rel,fill=freq))+ geom_tile()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
###### The median dataset performance of the "plausible" sets of features for each method
...
set.seed(644545)
seeds_glob=sample.int(213212,4,replace=FALSE)
names(seeds_glob)= METHODS[1:4]
for (METHOD in METHODS[1:4]){
if (METHOD=='u') sel_feats<- names(features_AP_filtered90["u", features_AP_filtered90["u",] > 450 ])
if (METHOD=='md1D') sel_feats<-names(features_AP_filtered90["md1D", features_AP_filtered90["md1D",] > 450 ]) 
if (METHOD=='md12D') sel_feats<- names(features_AP_filtered90["md12D", features_AP_filtered90["md12D",] > 450 ]) 
 #Reduce(union, list(global_MDFS$res_2Dm$rel_set, 

 #global_MDFS$res_1Dm$rel_set, 

 #global_MDFS$res_1D0$rel_set, 

 #global_MDFS$res_2D0$rel_set)) 
if (METHOD=='md12Dp') sel_feats<- names(features_AP_filtered90["md12Dp", features_AP_filtered90["md12Dp",] > 450 ]) 
# Reduce(union,list(global_MDFS$res_2Dm$rel_set, 
#					global_MDFS$res_1Dm$rel_set, 
#					global_MDFS$res_1D0$rel_set, 
#					global_MDFS$res_2D0$rel_set,
#					global_MDFS$res_2D0$partner_set,
#					global_MDFS$res_2Dm$partner_set
#					)) 

set.seed(seeds_glob[[METHOD]])
RF_mdl<- randomForest(x=taxonomy.mean.final[,sel_feats],
		      y= decision.mean.final |> as.factor() )
    y_thr= balanced.decision(x = RF_mdl$votes[,2],
                             y= decision.mean.final )$threshold
    y_pbin<- (RF_mdl$votes[,2] >= y_thr)
    CONFxMETHOD[[METHOD]]=table(Predicted=as.numeric(y_pbin),Actual= decision.mean.final)
    ORxMETHOD[[METHOD]]=odds_ratio(y_pred = y_pbin,y = decision.mean.final)
    roc_curve=roc(response=decision.mean.final,
        predictor=RF_mdl$votes[,2],quiet=TRUE)
METHODxROC[[METHOD]]= roc_curve
AUCxROC[[METHOD]]= auc(roc_curve)
}
names(METHODxROC)<- c("MDFS-1D","MDFS-2D", "MDFS-2D + partners", "U-test")
...
P<-ggroc(METHODxROC, legacy.axes = T,aes=c("color"),lwd=0.75) +
geom_abline(slope = 1 ,intercept = 0, linetype="dashed") + # add identity line
theme(
panel.background = element_blank(),
axis.title.x = element_text(size =18, face = 'bold'),
...
scale_y_continuous(breaks = seq(0,1,0.25), labels = seq(0,1,0.25) * 100) +
  ...
  scale_color_discrete(name = "FS method") +
   guides(color = guide_legend(override.aes = list(lwd = 5)))
ggsave("figures/roc_agp.pdf", P, width=6, height=4)
print(P)
for (m in METHODS[1:4]){
	print(m)
	print(CONFxMETHOD[[m]])
	print(sprintf("OR=%.3f ",ORxMETHOD[[m]]))
}
```
...
PAIRWISE_TAB <- list()
nm_methods <- names(METHODxROC)

k <- 1
for (i in seq_along(nm_methods)) {
  for (j in seq_along(nm_methods)) {
    if (i < j) {
	...
	auc1<-temp
      ...
      rt <- roc.test(roc1, roc2, conf.int=TRUE)   # DeLong test


      # CI for delta AUC

      delta_ci_txt <- sprintf(
        "[%.3f, %.3f]",
	rt$conf.int[1],
	rt$conf.int[2]
      )

      PAIRWISE_TAB[[k]] <- data.frame(
        model_1 = gsub(",.AU.*", "", m1),
        model_2 = gsub(",.AU.*", "", m2),
        ...
        OR_1 = round(ORxMETHOD[[METHODS[i]]], 3),
        OR_2 = round(ORxMETHOD[[METHODS[j]]], 3),

        deltaAUC_CI = delta_ci_txt,

        Z = round(as.numeric(rt$statistic), 3),
        p_value = round(rt$p.value, 8),
...
PAIRWISE_TAB <- do.call(rbind, PAIRWISE_TAB)
...
df_to_latex_table <- function(df,
  ...
  # Column alignment: l for character, r for numeric
  align <- sapply(df, function(x)
    if (is.character(x)) "l" else "r"
  )
   colnames(df)-> clnames
  # Format contents
  df[] <- lapply(df, function(x) {
    if (is.numeric(x)) {
      if (is.null(digits)) {
        if (any(abs(x) < 1e-4, na.rm=TRUE))
        format(signif(x,4), trim = TRUE, scientific = TRUE)
       else
      ...
      gsub("_", "\\\\_", x)   # escape LaTeX underscores
    } else {
      x
    }
  })
...
###### Pseudo observation based test on the AUC across repeats

```{r}


long_auc<- long_perf[ long_perf$Metric== "AUC", ]
long_auc<- long_auc [ long_auc$Method!="md12DpInt",]
long_auc$Method<- as.character(long_auc$Method)

friedman.test(Value ~ Method | Repeat, data=long_auc) -> fried_Test
fried_Test
...
pairwise.wilcox.test(
  long_auc$Value[ long_auc$Repeat < 351], long_auc$Method[ long_auc$Repeat < 351],
  paired = TRUE,
  p.adjust.method = "holm"
)-> pwtest_res
pwtest_res
...
```{r}
pwtest_df<-pwtest_res$p.value |> as.data.frame() 
pwtest_df$rowheaders<- rownames(pwtest_res$p.value)
rownames(pwtest_df)<- NULL
pwtest_df<- pwtest_df[, c(ncol(pwtest_df), 1:(ncol(pwtest_df)-1))]
df_to_latex_table(pwtest_df, caption="parwise wilcoxon tests on AUC of machine learning models using features chosen by each method.") 
...
methods_pairs<-combn(unique(long_auc$Method),2,simplify=FALSE)
m1Vm2<- matrix(nrow= length(unique(long_auc$Method)), #-1,
    ncol= length(unique(long_auc$Method)))#-1)
colnames(m1Vm2)<- unique(long_auc$Method)#[-1]
rownames(m1Vm2)<-unique(long_auc$Method)#[ - length(unique(long_auc$Method)) ]
for (mp in methods_pairs)
{ m1Vm2[ mp[[1]], mp[[2]] ]= mean(long_auc$Value[long_auc$Method== mp[[1]]] >= 
      long_auc$Value[long_auc$Method== mp[[2]]] )
m1Vm2[ mp[[2]], mp[[1]] ]= mean(long_auc$Value[long_auc$Method== mp[[2]]] >= 
      long_auc$Value[long_auc$Method== mp[[1]]] )


    }
df_m1Vm2<- as.data.frame(m1Vm2)
df_m1Vm2$rowheaders<- rownames(m1Vm2)
rownames(df_m1Vm2)<- NULL
df_m1Vm2<- df_m1Vm2[, c(ncol(df_m1Vm2), 1:(ncol(df_m1Vm2)-1))]
colnames(df_m1Vm2)[[1]]= "ph"
df_m1Vm2

```

```{r}
do.call(data.frame, aggregate(Value ~ Method, long_auc, function (x){
	   c(mean=mean(x),
	     sd=sd(x),
...
summary_auc<- summary_auc[ order(summary_auc$Value.mean, decreasing=TRUE),]
colnames(summary_auc)<- c("Name","Mean","SD","CI95_low","CI95_high","CI99_low","CI99_high"
)
summary_auc[, 2:ncol(summary_auc) ]<- round( summary_auc[, 2:ncol(summary_auc) ], 3)
summary_auc
summary_auc$`2.5-97.5%`<- paste0("[",summary_auc$CI95_low, ",",summary_auc$CI95_high,"]")
summary_auc$`0.5-99.5%`<- paste0("[",summary_auc$CI99_low, ",",summary_auc$CI99_high,"]")
summary_auc<- summary_auc[, !(colnames(summary_auc)%in%c("CI95_low","CI95_high","CI99_low","CI99_high"))] 

```


```{r}
methods_pairs<-combn(summary_auc$Name,2, simplify=FALSE)
do.call(rbind, lapply(methods_pairs, function(p){
x<- long_auc$Value[ long_auc$Method== p[[1]] ]
y<- long_auc$Value[ long_auc$Method== p[[2]] ]
d<- x- y
data.frame(Name= paste(p[[1]],"vs",p[[2]]),
	   Mean= mean(d),
	   SD=sd(d),
	     p2.5=quantile(d,0.025),
	     p97.5=quantile(d,0.975),
	     p0.5=quantile(d, 0.005),
	     p99.5=quantile(d, 0.995),
	     m1WinFreq= mean(d>0)
	     )


})
)-> pairwise_summary_auc
colnames(pairwise_summary_auc)<- c("m1 vs m2","Mean","SD","CI95_low","CI95_high","CI99_low","CI99_high", "m1>m2")
 pairwise_summary_auc[, !(colnames(pairwise_summary_auc)%in% c("m1 vs m2", "SD")) ]= round(pairwise_summary_auc[,  !(colnames(pairwise_summary_auc)%in% c("m1 vs m2", "SD")) ],3)
 pairwise_summary_auc[, "SD"]= round(pairwise_summary_auc[,"SD"],4)
m1geqm2<- pairwise_summary_auc[["m1>m2"]]
pairwise_summary_auc$`2.5-97.5%`<- paste0("[",pairwise_summary_auc$CI95_low, ",",pairwise_summary_auc$CI95_high,"]")
pairwise_summary_auc$`0.5-99.5%`<- paste0("[",pairwise_summary_auc$CI99_low, ",",pairwise_summary_auc$CI99_high,"]")
pairwise_summary_auc<- pairwise_summary_auc[, !(colnames(pairwise_summary_auc)%in%c("CI95_low","CI95_high","CI99_low","CI99_high", "m1>m2"))] 
pairwise_summary_auc[["m1>m2"]]<-m1geqm2

```

```{r}

pairwise_summary_auc[["m1 vs m2"]]<-gsub("partners","p",pairwise_summary_auc[["m1 vs m2"]])
summary_auc[["Name"]]<-gsub("partners","p",summary_auc[["Name"]])
summary_auc
pairwise_summary_auc
...
viz_fs_df$partner_freq<-  colMaxs( partners_m0[, match(fs_df$taxon, colnames(tdata)) ])
...
sel_freq<- viz_fs_df[,grep("rel_", colnames(viz_fs_df))]

taxa_sets<- lapply(sel_freq, function(x) viz_fs_df$taxon[ x >= 450 ] )

taxa_sets[["partners"]]<- sapply(strsplit(colnames(tdata)[ colMaxs(partners_m0) >= 450 ], "\\."), function(z) z[[length(z)]])
combins<- list(all= names(taxa_sets)[1:3],
	       oneD= c("rel_u", "rel_1D"),
	       MDFS_12D= c("rel_1D", "rel_2D"),
	       MDFS_1D=c("rel_1D"),
	       U_test= c("rel_u"),
	       MDFS_2D=c("rel_2D"),
	       partners=c("partners"))
sapply( combins, function(x)
       #if (length(x)>1)
       length(Reduce(intersect, taxa_sets[ x ] ))
       #else length(taxa_sets[[x]])
)
...
#| dependson: fs_df
#| cache: true
binarize_taxa(as.matrix(taxonomy.mean.final[,plausible]),
                thresholds = rep(0, length(plausible)))-> pres_final_plausible 
pab_df<- data.frame(taxon=plausible,
                        ab= colSums(taxonomy.mean.final[,plausible])/colSums(pres_final_plausible),
                        ab_SD= apply(abundance_per_rep,2,jackknife_SE, nrow(taxonomy.mean.final), 0.37* nrow(taxonomy.mean.final) ),
                        abH= colSums(taxonomy.mean.final[ decision.mean.final==0 ,plausible])/colSums(pres_final_plausible[ decision.mean.final==0,]),
                        abH_SD= apply(abundance_per_repH,2,jackknife_SE, sum(decision.mean.final==0), 
                                      0.37*sum(decision.mean.final==0)),
                        abD= colSums(taxonomy.mean.final[ decision.mean.final==1 ,plausible])/colSums(pres_final_plausible[ decision.mean.final==1,]),
                        abD_SD= apply(abundance_per_repH,2,jackknife_SE, sum(decision.mean.final==1), 
                                      0.37*sum(decision.mean.final==1)),
                        pr=colMeans(pres_final_plausible),
                        pr_SD=apply(presence_0,2, jackknife_SE, nrow(taxonomy.mean.final),
                                    0.37*nrow(taxonomy.mean.final)),
                        prH=colMeans(pres_final_plausible[decision.mean.final==0,]),
                        prH_SD=apply(presence_0H,2,jackknife_SE, sum(decision.mean.final==0), 
                                      0.37*sum(decision.mean.final==0)),
                        prH=colMeans(pres_final_plausible[decision.mean.final==1,]),
...
viz_pab_df[,2:ncol(viz_pab_df)]<- round(viz_pab_df[,2:ncol(viz_pab_df)],3)
...
viz_ig_pab<-cbind( viz_fs_df[order(-viz_fs_df$IG2D),],
       viz_pab_df[order(-viz_fs_df$IG2D) , 2:ncol(viz_pab_df) ])
colnames(viz_ig_pab)<- c("taxon", "IG2D", "IG2D_SD", "IG1D", "IG1D_SD", "selection_freq_1D", 
			 "selection_freq_2D", "selection_freq_U_test", "abundance", "abundance_SD",
			 "abundance_H", "abundance_H_SD", "abundance_D", "abundance_D_SD", 
			 "presence(zero_based)", "presence_SD", "presence_H", "presence_H_SD",
			 "presence_D", "presence_D_SD")
write.csv(viz_ig_pab,
...
publication_tab<- viz_ig_pab[, c("taxon", "IG2D", "IG1D", "selection_freq_1D", "selection_freq_2D", "selection_freq_U_test", 
				 "abundance", "presence(zero_based)",  
			 "abundance_H", "abundance_D",
			 "presence_H",  "presence_D")
				  ]

colnames(publication_tab)<- c("taxon","IG2D", "IG1D", "MDFS1D_freq", "MDFS2D_freq", "Utest_freq", "abundance", "presence", 
			 "abundance_H", "abundance_D",
			 "presence_H",  "presence_D")
...
```{r}
morans_I<- function( S, y) {
 n<- length(y)
 s_o<- sum(S)
 y<- y - mean(y)
 y_o<- y %o% y
 (n/s_o)*sum(S*y)/sum( diag(y_o) ) 
}


morans.test<- function(S,y){
I_obs<- morans_I( S,y)
I_perm<- replicate(2000, {
  y_perm<- sample(y)
  morans_I(S, y_perm)

  })
list(p.value= mean(abs(I_perm) >= abs(I_obs)),
     statistic= I_obs)
}
```
...
for (name in names(eFPR)){
	cat(sprintf("\nsummary for %s method:\n  percentage of selected variables with permuted decision \n  per permutation repeat, highest percentiles", 
		      nameMap[[name]]))
	cat(paste0("\n    ", capture.output(
					      round(quantile(eFPR[[name]], probs=seq(0.93,1,0.02)),4)
...
```{r}
pdf("figures/max_perm_IG.pdf")
for (name in names(max_permIG)[1:2])
 hist(max_permIG[[name]],main=sprintf("maximum %s - distribution over permutations", name))
dev.off()
for (name in names(max_permIG)[1:2])
 hist(max_permIG[[name]],main=sprintf("maximum %s - distribution over permutations", name))
for (name in names(max_permIG)[1:2]){
	cat(sprintf("\n percentiles of maximum %s observed - over permuations", name))
	cat(paste0("\n    ", capture.output(round(quantile(max_permIG[[name]], probs=c(0.9,0.95,0.98,0.99,0.998,1)),2 ))))
}
```


###### Delta IG - for confirmed variables that had some confirmed synergistic partners
...
perm_DELTA_IGs<- matrix(nrow=2000, ncol=length(plausible_synergizers))
for (j in 1:2000){
perm_DELTA_IGs[j,]= readRDS(sprintf(deltaIG_pattern,j,lop))
}
colnames(perm_DELTA_IGs)<- plausible_synergizers
colnames(perm_DELTA_IGs)<- sapply(strsplit(colnames(perm_DELTA_IGs),"\\."), function(x) x[[length(x)]])
```


```{r}
apply(perm_DELTA_IGs,2,function(x)
      quantile(x, probs=c(0.9,0.95,0.98,0.99,0.998,1)) |> round(2)

) |> t() |> as.data.frame() -> delta_IG_permdf

delta_IG_permdf$taxon<- rownames(delta_IG_permdf)
rownames(delta_IG_permdf)<- NULL
delta_IG_permdf<- delta_IG_permdf[,c(ncol(delta_IG_permdf),
				     1:(ncol(delta_IG_permdf)-1))]
delta_IG_permdf
...
plot_survival_function( n_perm_gt_IG1D0, sort(mean_subsampleIG1D), 
			...
			crit_x_annot= 'min IG1D \n (confirmed)',
		       ...
		       ylab='# perm_IG > IG[i]')
min(mean_subsampleIG1D[ fs_df$rel_1D >= 450 ]) |> print()
...
plot_survival_function( n_perm_gt_IG2D0, sort(mean_subsampleIG2D), log='x', 
			...
			crit_x_annot= 'min IG2D \n (confirmed)',
		       ...
		       xlab='IG[i] (jackknife mean of confirmed variable number i )',
		       ylab='# perm_IG > IG[i]')
min(mean_subsampleIG1D[ fs_df$rel_2D >= 450 ]) |> print()

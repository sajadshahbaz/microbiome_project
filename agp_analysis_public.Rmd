...
```{r}
overlaps<-matrix(nrow=length(subsample), ncol=length(subsample))
for (i in seq_along(subsample))
for (j in seq_along(subsample)) {
 if (i < j){
overlaps[i,j]<-overlaps[j,i]<- length(intersect(subsample[[i]]$keep,subsample[[j]]$keep))/
	       length(union(subsample[[i]]$keep,subsample[[j]]$keep))
 }
 overlaps[i,i]= 1
}

L=  diag(colSums(overlaps)) - overlaps
fiedler= eigen(L)$vectors[, length(subsample) -1 ]
long_perf$fielderOrder= rank(fiedler)[ long_perf$Repeat ]
hc_order= hclust( as.dist(1-overlaps), method="average")$order
long_perf$hcOrder= hc_order[ long_perf$Repeat ]
...
wide <- dcast(
  long_perf,
  Repeat + Metric + fielderOrder + hcOrder ~ Method,
  value.var = "Value"
)
pairs_methods<- 	t(combn(unique(long_perf$Method),2))

diff_long <- do.call(
  rbind,
  lapply(seq_len(nrow(pairs_methods)), function(i) {
    m1 <- as.character(pairs_methods[i, 1])
    m2 <-  as.character( pairs_methods[i, 2])
    ...
    data.frame(
      ...
      Method1      = m1,
      Method2      = m2,
      Diff         = wide[[m2]] - wide[[m1]],
      fielderOrder = wide$fielderOrder,
      hcOrder      = wide$hcOrder,
...
diff_long$Comparison <- paste(diff_long$Method2,
                              "–",
                              diff_long$Method1)
...
ggplot(diff_long,
  ...
  geom_point(size = 0.6) +
  facet_wrap(Comparison~ Metric, scales = "free_y") +
  theme_bw()
...
pairwise.wilcox.test(
  long_auc$Value[ long_auc$Repeat < 351], long_auc$Method[ long_auc$Repeat < 35t1],
  paired = TRUE,
  p.adjust.method = "holm"
)-> pwtest_res
pwtest_res

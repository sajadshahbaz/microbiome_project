...
#### U_test vs taxa binarized differently

```{r}
print("taxa found only by U test and discretized differently")
data.frame(name=U_test_only_taxa, different_discr= U_test_only_taxa %in% common_taxa)
```
```{r}
xx$adjusted.p.value[xx$relevant.variables]-> U_test_pvs
names(U_test_pvs)<-xx$rel_set
data.frame(names=lapply(U_test_only_taxa, get_deepest_taxonomy_lvl) |> unlist() |> unname(),
            IG_1D= MDFS_object_lists[[1]][[1]]$statistic[ match(U_test_only_taxa,colnames(base_resamples[[1]]$taxa_binary) ) ],
           IG_2D= MDFS_object_lists[[2]][[1]]$statistic[ match(U_test_only_taxa,colnames(base_resamples[[1]]$taxa_binary) ) ],
U_pv=U_test_pvs[U_test_only_taxa],
different_discr= U_test_only_taxa %in% common_taxa,
freq=  colMeans(base_resamples[[1]]$taxa_binary[,U_test_only_taxa])
      )-> utest_only_summary
rownames(utest_only_summary)<-NULL
t(utest_only_summary)
```

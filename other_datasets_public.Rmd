...
meta_OK<- metadata[ metadata$cohort %in% cohort_ids, ]
do.call(rbind, lapply(cohort_ids, function(coh)
       { 
       tab<-table(meta_OK$category[ meta_OK$cohort==coh ])
       NAMES<- c("healthy")
       VALUES<- as.character(c(tab[["healthy"]]))
       tab<- tab[ names(tab)!=c("healthy") ]
       NAMES[[2]]<-paste0(names(tab), collapse="|")
       VALUES[[2]]<- paste0(tab, collapse="|")
       rbind(NAMES,VALUES, c(VALUES[[1]], sum(tab)), c(sum(tab, VALUES[[1]] |> as.integer()),"") )
       }
))-> tab_sampleSizes

rownames(tab_sampleSizes)<- unlist(lapply(cohort_ids, function(x) c(x,"","totals", "")))
colnames(tab_sampleSizes)<- c("0","1")
tab_sampleSizes
...
method_selected<- array(0, dim=c(3,4,ncol(tdata)),
			dimnames=list(cohort=names(features[[1]]),
				      method=c("U-test", "MDFS1D", "MDFS2D", "partners"),
				      taxon= colnames(tdata)
			)
			)
method_name_map<- data.frame( features_name= names(features),
			      msel_name= dimnames(method_selected)$method)

for (coh in names(features[[1]]))
  for (j in seq_along(features[[1]][[coh]]))

  {
  MDFS1d<- features[[ "md1D" ]][[coh]][[j]]
  MDFS2d<- union(full_MDFSres[[coh]][[j]]$res_2Dm$rel_set,
full_MDFSres[[coh]][[j]]$res_2D0$rel_set)
  partners<- setdiff(features[["md12Dp"]][[coh]][[j]], union(MDFS1d, MDFS2d) )
 method_selected[ coh, "MDFS1D", ] = method_selected[ coh,"MDFS1D", ] +  (colnames(tdata) %in% MDFS1d)
 method_selected[ coh, "MDFS2D", ] = method_selected[ coh,"MDFS2D", ] +  (colnames(tdata) %in% MDFS2d)
 method_selected[ coh, "partners", ] = method_selected[ coh,"partners", ] +  (colnames(tdata) %in% partners)
 method_selected[ coh, "U-test", ] = method_selected[ coh,"U-test", ] +  (colnames(tdata) %in% features[["u"]][[coh]][[j]])

  }

combins<- list(all= setdiff(method_name_map$msel_name, "partners"),
	       OneD= c("U-test", "MDFS1D"),
	       MDFS_12D= c("MDFS1D", "MDFS2D"),
	       MDFS_1D= c("MDFS1D"),
	       U_test=c("U-test"),
	       MDFS_2D= c("MDFS2D"),
	       partners=c("partners"))
cohXcbn_n_sel<- array(NA, dim=c(length(cohort_ids), length(combins)),
                          dimnames=list(cohort=cohort_ids,
					method_combinations=names(combins)))

for (coh in cohort_ids){
  for (cb_name in names(combins)){
  cbn<- combins[[cb_name]] 
  ker<- dimnames(method_selected)[[2]] %in% cbn 
  cohXcbn_n_sel[coh, cb_name]= apply(method_selected[coh,,] >= 450, 2, function(x) all(x==ker))  |> sum()
  }
}
...
```{r}
roc_cohXmethod<- rep(NA , 3) |> as.list()
conf_cohXmethod<- array(NA, dim=c(3, 2, 2,2),
			dimnames=list(cohort=names(features_AP_90),
				      method=c("md1D", "md12Dp"),
				      y_pred=NULL,
				      y_true=NULL
				      ))
auc_cohXmethod<- array(NA, dim=c(3, 2),
			dimnames=list(cohort=names(features_AP_90),
				      method=c("md1D", "md12Dp")))
OR_cohXmethod<- array(NA, dim=c(3, 2),
			dimnames=list(cohort=names(features_AP_90),
				      method=c("md1D", "md12Dp")))

names(roc_cohXmethod)<- names(features_AP_90)
for (coh in names(features_AP_90)) {
 roc_cohXmethod[[coh]]=list()
 for (meth in c("md1D", "md12Dp"))
 roc_cohXmethod[[coh]][[meth]]= NA
}
for (coh in names(features_AP_90)) 
 for (meth in c("md1D", "md12Dp")){
   fs<- colnames(features_AP_90[[coh]])[ 
           features_AP_90[[coh]][meth,] >= 450
	   ]
   if (length(fs)){
        X<- taxa[[coh]][, fs , drop=FALSE]
        y<- decision[[coh]]
set.seed(1234)
        print(sprintf("%s %s",coh, meth))
        print(paste("    ", fs))
        rf_mdl<- randomForest(x=X, y=as.factor(y) )
       y_thr= balanced.decision(x = rf_mdl$votes[,2],
                             y= y)$threshold
       y_pbin<- (rf_mdl$votes[,2] >= y_thr)
       stopifnot(length(unique(y_pbin))>1)
       conf_cohXmethod[coh, meth, ,]= table(Predicted=as.numeric(y_pbin),
					    Actual= y)
       OR_cohXmethod[coh, meth]=odds_ratio(y_pred=y_pbin,
					   y=y)
       auc_cohXmethod[coh, meth]= auc(response=y,
				      predictor=rf_mdl$votes[,2],
				      quiet=TRUE)
       roc_cohXmethod[[coh]][[meth]]=roc(response=y,
					 predictor=rf_mdl$votes[,2],
       quiet=TRUE)
   }
...
roc.test(roc_cohXmethod[[1]][[1]], roc_cohXmethod[[1]][[2]],
	 conf.int=TRUE)

...
## convert raw MDFS result to taxa names
...
## for each cohort, select results from both binarization methods and save them
## as a separate entry of the list
...
## intersection_matrix[i,j]= number of taxa found in both sets_list[[i]] and sets_list[[j]]
...
## Given input dataset and a vector of indexes of interesting taxons,
## function returns a data.frame, one row per one interesting taxon,
## that contains ID of other taxa that increases the information on decision gained from
## adding other taxa to the interesting taxon the most.
...
Here, we inspect taxa with highest IG in 2D analysis. For the case of these datasets, all of them were found relevant in 1D analysis, meaning that the true synergistic effect here is unlikely.
As a reference, we compare increase of IG as we go from 1D to 2D analysis with minimum IG obtained in 1D and none of the taxa found by picking the highest IG 2D pass such a filter:
...
In contrast, there are few taxons that were found relevant only in 2D analysis and applying the same criterion on deciding wether the observed interaction is truly synergistic or not, all of them pass the test (the increase of IG from 1D and 2D surpasses the minimal IG in 1D).
...
We run Random Forest classifiers using the taxa abundance and equip them with variables found relevant by each of the feature selection methods tested.

For MDFS, we consider a variable to be relevant here if it was found relevant by either of the methods.
...
- leave random 10% of the samples out (keep it as Out Of Bag (OOB) data)
...
Below function streamlines MDFS analysis for one trial (accepting abundance as `X`, and decision vector as `y`, while `mc` determines mininum allowable number of samples in each binarized category of the taxon - taxa that have less than `mc` samples in any class after binarization are not used).
...
Apply `U_test_FS` and `MDFS_FS` once for testing purposes.
...
##### Random forest and feature selection in repeated resampling

Held out data from each run is then used for testing performance of the classifier:
- area under ROC curve (AUC),
- diagnostic odds ratio (OR)

...are computed on each trial and the distribution and spread of each statistic is reported.

First, do the heavy lifting : model training and feature selection, save intermediate results for later:
...
subsampleSize=0.9
for (cohort in names(taxa))
  FS[[cohort]]<-mdfs[[cohort]]<-ut[[cohort]]<-list()
for (j in 1:ntry){
  set.seed(j)
  # construct ntry resamples, save them as indexes of samples to keep from each cohort
  lapply(decision, function(taxa_cohort){
    y1idx<- which(taxa_cohort==1 )
    y0idx<- which(taxa_cohort==0 )
    c(sample(y1idx, subsampleSize*length(y1idx),replace=FALSE),
      sample(y0idx, subsampleSize*length(y0idx),replace=FALSE))
  }
  )-> B_idx
  resample_IDX[[j]]=B_idx
  RF_coh<-list()
  for (cohort in names(taxa)){
    ...
    ut[[cohort]][[j]]<-res_u
    FS[[cohort]][[j]]<- list()
    for (mdv in names(res_mdfs)) FS[[cohort]][[j]][[mdv]]= res_mdfs[[mdv]]$rel_set
    FS[[cohort]][[j]][["u_test"]]= res_u$rel_set
    RF_models_file=sprintf("RF_models/%s_rep_%d_RF.rds",cohort,j)
    if (!file.exists(RF_models_file)) {
      set.seed(j)
      message("computing RF...")
      RF_models= list()
      for (mdv in names(res_mdfs)){
        if (length(res_mdfs[[mdv]]$relevant.variables))
          RF_models[[mdv]]= randomForest(x=taxa[[cohort]][B_idx[[cohort]], FS[[cohort]][[j]][[mdv]] ,drop=FALSE  ],
                                         y=as.factor( decision[[cohort]][B_idx[[cohort]] ])
          )
        message(mdv); gc()
      }
      if (length(res_u$relevant.variables))
        RF_models[["u_test"]]=randomForest(x=taxa[[cohort]][B_idx[[cohort]], res_u$rel_set,drop=FALSE  ],
                                           y=as.factor(decision[[cohort]][B_idx[[cohort]] ])
        )
      saveRDS(RF_models,RF_models_file); gc()
    } else {
      RF_models<-readRDS(RF_models_file)
    }
    RF_coh[[cohort]]<- RF_models
  }
  RF[[j]]=RF_coh
  message(j)
...
Compute said statistics based on saved results:

```{r}
source("balanced_decision.R")
COHORTS= names(taxa)
temp<-lapply(RF[[1]], names)
METHODS=temp[[which.max(sapply(temp,length)) ]] 
METRICS=c("AUC","OR")
odds_ratio<- function(y_pred, y, method, cohort){
...
performance=array(dim = c(length(COHORTS), length(METHODS),length(METRICS), length(RF)),
                  dimnames = list(COHORTS,
                                  METHODS,
                                  METRICS,
                                  NULL  #30 repeats
                                  )
...
pred_thr= array(NA, dim= c(length(COHORTS), length(METHODS), length(RF)),
                 dimnames = list(COHORTS,
                                  METHODS,
                                  NULL  #30 repeats
                                  )
...
for (j in seq_along(RF)){
  ...
  RF[[j]][[coh]][[method]]-> RF_mcj
   ...
   # print(sprintf("%d %s %s ok",j,method,coh) )
    test_pred= predict(RF_mcj, newdata=  taxa[[coh]][ -resample_IDX[[j]][[coh]],
                                                 FS[[coh]][[j]][[method]] ],type="prob"  )[,2]
    y_thr= balanced.decision(x = RF_mcj$votes[,2], 
                             y= decision[[coh]][ resample_IDX[[j]][[coh]] ])$threshold
    pred_thr[coh,method,j]=y_thr
    y_pbin<- (test_pred >= y_thr)
    confusion=table(Predicted=y_pbin,Actual=decision[[coh]][-resample_IDX[[j]][[coh]] ])
    performance[coh,method,"AUC",j]= auc(response=decision[[coh]][-resample_IDX[[j]][[coh]] ],
                                        predictor=test_pred,quiet=TRUE)|> as.numeric() 
    performance[coh,method,"OR",j]=  odds_ratio(y_pbin, decision[[coh]][-resample_IDX[[j]][[coh]] ],
                                                method, coh)
...
par(mar = c(5, 13, 4, 1)) 
#par(mfrow=c(1,2))
do.call(rbind, lapply(names(decision),
                      function(nm) { tab<-performance[nm,,"AUC",]
                      rownames(tab)<-gsub("res_", "", rownames(tab))
                                      rownames(tab)<-paste0(nm,"_",rownames(tab)); tab}
                      ) )-> AUC_all
do.call(rbind, lapply(names(decision),
                      function(nm) {tab<-performance[nm,,"OR",]
                      rownames(tab)<-gsub("res_", "", rownames(tab))
                        rownames(tab)<-paste0(nm,"_",rownames(tab)); tab}
                      ) )-> OR_all

boxplot(t(AUC_all),las=2, horizontal=TRUE, main="classifier OOB AUC")
temp<-t(OR_all)
rownames(temp)<-NULL
boxplot(temp[,1:5],las=2, horizontal=TRUE, main="classifier OOB OR",ylim=c(0,25))
boxplot(temp[,6:10],las=2, horizontal=TRUE, main="classifier OOB OR",ylim=c(0,25))
boxplot(temp[,11:15],las=2, horizontal=TRUE, main="classifier OOB OR")
```


##### Random forest and feature selection in repeated resampling 0.8
...
for (j in 1:ntry){
  ...
  for (cohort in names(taxa)){
    ...
    for (mdv in names(res_mdfs)) FS[[cohort]][[j]][[mdv]]= res_mdfs[[mdv]]$rel_set
    FS[[cohort]][[j]][["u_test"]]= res_u$rel_set
    RF_models_file=sprintf("RF_models/%s_rep_%d_RF_%.2f.rds",cohort,j,subsampleSize)
    if (!file.exists(RF_models_file)) {
      set.seed(j)
      message("computing RF...")
      RF_models= list()
...
for (j in seq_along(RF)){
  ...
  RF[[j]][[coh]][[method]]-> RF_mcj
   ...
   # print(sprintf("%d %s %s ok",j,method,coh) )
   # print(dim(taxa[[coh]][ -resample_IDX[[j]][[coh]],
  #                           FS[[coh]][[j]][[method]], drop=FALSE ]))
    test_pred= predict(RF_mcj, newdata=  taxa[[coh]][ -resample_IDX[[j]][[coh]],
                                                 FS[[coh]][[j]][[method]],
                                                 drop=FALSE], type="prob"  )[,2]
    y_thr= balanced.decision(x = RF_mcj$votes[,2], 
                             y= decision[[coh]][ resample_IDX[[j]][[coh]] ])$threshold
    pred_thr[coh,method,j]=y_thr
    y_pbin<- (test_pred >= y_thr)
...
rownames(temp)<-NULL
boxplot(temp[,1:5],las=2, horizontal=TRUE, main="classifier OOB OR",ylim=c(0,25))
boxplot(temp[,6:10],las=2, horizontal=TRUE, main="classifier OOB OR",ylim=c(0,15))
boxplot(temp[,11:15],las=2, horizontal=TRUE, main="classifier OOB OR")
```


##### Random forest and feature selection in repeated resampling 0.68
...
rownames(temp)<-NULL
boxplot(temp[,1:5],las=2, horizontal=TRUE, main="classifier OOB OR",ylim=c(0,25))
boxplot(temp[,6:10],las=2, horizontal=TRUE, main="classifier OOB OR",ylim=c(0,10))
boxplot(temp[,11:15],las=2, horizontal=TRUE, main="classifier OOB OR",ylim=c(0,7))
```
...
```{r}
cbind(pred_thr["YachidaS_2019","res_1D0",], performance["YachidaS_2019","res_1D0","OR",])-> thr_OR

 plot(thr_OR, xlab="thr", ylab="OR", main="RF performance on 1D0 feature set across 30 trials")
```

```{r}

bad_trials<-(thr_OR[,1]<0.1)

for ( i in which(bad_trials)){
  X_train=taxa$YachidaS_2019[ resample_IDX[[i]]$YachidaS_2019, FS$YachidaS_2019[[i]]$res_1D0 ]
   X_test=taxa$YachidaS_2019[ -resample_IDX[[i]]$YachidaS_2019, FS$YachidaS_2019[[i]]$res_1D0 ]
   y_train=decision$YachidaS_2019[resample_IDX[[i]]$YachidaS_2019 ]
   y_test= decision$YachidaS_2019[-resample_IDX[[i]]$YachidaS_2019 ]
   rf_model_i<- RF[[i]]$YachidaS_2019$res_1D0
   balanced.decision(x = predict(rf_model_i, newdata=X_train, type="prob")[,2],
                     y= y_train)-> balanced_train_res
   y_pred_test_prob= predict(rf_model_i, newdata= X_test,type="prob")[,2]
   y_pred_test_dec= y_pred_test_prob >= balanced_train_res$threshold
   print("_______________________")
   print("_______________________")
   print("balanced decision thr on train set:")
   print(balanced_train_res$threshold)
   print("confusion on test set using above thr:")
   print(table(predicted=y_pred_test_dec, 
               actual= y_test))
   print(odds_ratio(y_pred_test_dec, 
                    y_test))
   y_pred_test_builtin= predict(rf_model_i, newdata= X_test)
   print("results using builtin thr from RF: ")
   print(table(predicted=y_pred_test_builtin, 
               actual= y_test))
   print(odds_ratio(y_pred_test_builtin, 
                    y_test))
...
As a final, high confidence result, we keep features that were found relevant at each repeat of the resampling procedure.
...
for (cohort in COHORTS){
  consensus_features[[cohort]]<-list()
  for (method in METHODS){
  consensus_features[[cohort]][[method]]= FS[[cohort]][[1]][[method]]
  for (j in 2:ntry)
    consensus_features[[cohort]][[method]]= intersect(consensus_features[[cohort]][[method]],
                                                      FS[[cohort]][[j]][[method]])
  }
}
```
...
for (cohort in names(consensus_features)){
  names(consensus_features[[cohort]])->fs_names
  grep("D",fs_names)->mdfs_mask
  consensus_features[[cohort]][["any_mdfs"]]= Reduce(union,consensus_features[[cohort]][mdfs_mask] )
  grep("2D",fs_names)->twoD_mask
  grep("1D",fs_names)->oneD_mask
  Reduce(union, consensus_features[[cohort]][oneD_mask])-> set_1D
  Reduce(union, consensus_features[[cohort]][twoD_mask])-> set_2D
  consensus_features[[cohort]][["only_2D"]]=setdiff(set_2D,set_1D)
  print(cohort)
  print(intersection_matrix(consensus_features[[cohort]][1:5] ))
  print(intersection_matrix(consensus_features[[cohort]][5:7] ))
}

```

##### Nogueira et al. (2018) FS stability:

```{r}

selection_indicator_matrix<-function(selection_list, all_features){
  
  Z<- matrix(nrow=length(selection_list),ncol=length(all_features))
  for (i in seq_along(selection_list))
    Z[i,]= all_features %in% selection_list[[i]]
  Z
}

nogueira_stability<- function(selection_list, all_features){
  Z<- selection_indicator_matrix(selection_list = selection_list,all_features = all_features)
  p_f<- colMeans(Z)
  mean(rowSums(Z))->k_
  d=ncol(Z)
  M=nrow(Z)
...
stability<-matrix(nrow=length(COHORTS), ncol=length(METHODS))
rownames(stability)<- COHORTS
colnames(stability)<- METHODS
for (cohort in COHORTS){
for (method in METHODS){
  print(sprintf("%s %s",cohort, method))
 lapply(1:ntry, function(j) FS[[cohort]][[j]][[method]])-> FS_list_cm
  stability[cohort,method]=nogueira_stability(selection_list = FS_list_cm,
                     all_features= colnames(taxa[[cohort]]))
}
}

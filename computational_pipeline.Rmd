...
odds_ratio<- function(y_pred, y, method, cohort){
  ...
  table(Predicted=y_pred,Actual=y)+0.5-> CM
...
for (j in seq_along(RF)){
  ...
  RF[[j]][[coh]][[method]]-> RF_mcj
   ...
   # print(sprintf("%d %s %s ok",j,method,coh) )
    ...
    train_pred= predict(RF_mcj, newdata=  taxa[[coh]][ resample_IDX[[j]][[coh]],
                                                  FS[[coh]][[j]][[method]]  ],type="prob"  )[,2]
    y_thr= balanced.decision(x = train_pred, y= decision[[coh]][ resample_IDX[[j]][[coh]] ])$threshold
...
temp<-t(OR_all)
rownames(temp)<-NULL
boxplot(temp[,1:10],las=2, horizontal=TRUE, main="classifier OOB OR")

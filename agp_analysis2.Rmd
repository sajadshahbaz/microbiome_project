...
frequent_<- list(over95= 475, over90=450)
...
```
```{r}
colnames(partners_per_rep)<- plausible
...
```{r}
library(knitr)
bacteria_only_2D<-fs_df$taxon[(fs_df$rel_2D>450)&(fs_df$rel_1D<450)]

partners_only2D<- sapply(bacteria_only_2D,
                         function(taxon)
                           names(table(partners_per_rep[,taxon]))[[which.max(table(taxon))]]
                           )
partners_only2D_freq<- sapply(bacteria_only_2D,
                         function(taxon)
                           table(partners_per_rep[,taxon])[[which.max(table(taxon))]] |> unname()
                           )

# for (i in seq_along(bacteria_only_2D)){
#   
#   print(sapply(strsplit(c(bacteria_only_2D[[i]],
#           partners_only2D[[i]]),split="\\."
#           ), function(x) x[[length(x)]]))
#   print(sprintf("partnering frequency: %d", partners_only2D_freq[[i]] ))
#   print(
#   table(
#         rel=(taxonomy.mean.final[,bacteria_only_2D[[i]]]>0)*1,
#         partner=(taxonomy.mean.final[,partners_only2D[[i]]]>0)*1,
#         y=decision.mean.final
#         )
#   )
#   
# }
...
for (i in seq_along(bacteria_only_2D)) {
  ...
  bacteria_name <- tail(strsplit(bacteria_only_2D[[i]], "\\.")[[1]], 1)
  partner_name <- tail(strsplit(partners_only2D[[i]], "\\.")[[1]], 1)
  
  # Print header
  cat("\n\n---\n")
  cat(sprintf("### Pair %d: %s vs %s\n", i, bacteria_name, partner_name))
  
  # Print frequency
  cat(sprintf("**Partnering frequency**: %d\n\n", partners_only2D_freq[[i]]))
  
  # Compute binary presence matrix
  rel <- as.integer(taxonomy.mean.final[, bacteria_only_2D[[i]]] > 0)
  partner <- as.integer(taxonomy.mean.final[, partners_only2D[[i]]] > 0)
...
viz_fs_df2$partnership_freq=sapply(plausible,
                         function(taxon){ if (taxon %in% colnames(partners_per_rep))
                           unname(table(partners_per_rep[,taxon])[[which.max(table(taxon))]])
                           else "foo"
                         }
                    )
fs_df2<- viz_fs_df2

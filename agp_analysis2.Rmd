...
for (j in 1:n_repeats){
  ...
  for (fs_method in names(features)){
    ...
    if (!file.exists(fname)){
      ...
      if (length(features[[fs_method]][[j]])) {
      RF_fs_j= randomForest(x=  tdata[ subsample[[j]]$keep, features[[fs_method]][[j]],
                                      drop=FALSE
                                      ],
                            y= y[   subsample[[j]]$keep ] |> as.factor() ) 
      }
...
for (j in 1:n_repeats){
  ...
  no_features<- (length(RF_mj)==1)
    ...
    test_pred= predict(RF_mj, newdata=  tdata[ subsample[[j]]$leave,
                                                 features[[method]][[j]],
                                                 drop=FALSE
                                                 ],type="prob"  )[,2]
...
```
```{r}
only_highest_lvl<- function(t_names){
  sapply(strsplit(t_names,"\\."),function(x) x[[length(x)]])
  
}
all_12Dp<-Reduce(intersect,features$md12Dp) |> only_highest_lvl()
all_12D<-Reduce(intersect,features$md12D) |> only_highest_lvl()
all_1D<-Reduce(intersect,features$md1D) |> only_highest_lvl()
all_u<-Reduce(intersect,features$u) |> only_highest_lvl()

universe<-Reduce(union, list(all_12D,all_1D,all_12Dp,all_u))

absence_presence<- matrix(nrow=4, ncol=length(universe))
rownames(absence_presence)<-c("u","all_1D","all_12D","all_12Dp")
colnames(absence_presence)<-universe
all_times_rel<-list(u=all_u,all_1D=all_1D,all_12D=all_12D,all_12Dp=all_12Dp)
for (met_name in names(all_times_rel)){
  absence_presence[met_name,] = universe %in% all_times_rel[[met_name]]
}

t(1*absence_presence)
